...
TRADER_COLOR = (118, 222, 148)
TRADER_POTION_COST = 350
TRADER_WEAPON_COST = 1000
POTION_HEAL_AMOUNT = 5
HIGH_RARITY_KILL_HEAL = 3
...
class GameView(arcade.View):
	...
	def __init__(self, sound_bank: SoundBank, starting_weapon: str, starting_weapon_rarity: int) -> None:
		...
		self.rest_floor = False
		self.trader_pos: tuple[int, int] = (-1, -1)
		self.trader_shop_open = False
		self.trader_weapon: str | None = None
		self.trader_weapon_rarity: int = 4
	...
	def setup_floor(self, reset_health: bool) -> None:
		...
		self.rest_floor = self.floor_number % 5 == 0 and not is_final_floor
		self.trader_pos = (-1, -1)
		self.trader_shop_open = False
		self.trader_weapon = None
		...
		elif self.rest_floor:
			reserved.add(self.exit_pos)
			far_tiles = [
				t for t in self.floor_tiles
				if t not in reserved and abs(t[0] - self.player_pos[0]) + abs(t[1] - self.player_pos[1]) >= 6
			]
			self.trader_pos = random.choice(far_tiles if far_tiles else [t for t in self.floor_tiles if t not in reserved])
			self.trader_weapon = random.choice(WEAPON_ORDER)
			self.trader_weapon_rarity = random.choice([4, 5])
			self.items = []
			self.enemies = []
		else:
			reserved.add(self.exit_pos)
	...
	def spawn_items(self, reserved_tiles: set[tuple[int, int]]) -> list[Item]:
		...
		for _ in range(max(4, 6 - self.floor_number // 4)):
			tile_x, tile_y = available_tiles.pop()
	...
	def on_draw(self) -> None:
		...
		self.draw_exit()
		self.draw_enemies()
		if self.rest_floor and self.trader_pos[0] >= 0:
			self.draw_trader()
		self.draw_player()
	...
	def on_key_press(self, symbol: int, modifiers: int) -> None:
		if self.trader_shop_open:
			if symbol == arcade.key.ESCAPE:
				self.trader_shop_open = False
				self.status_text = "You bid the trader farewell."
			elif symbol == arcade.key.KEY_1:
				self._buy_health_potion()
			elif symbol == arcade.key.KEY_2:
				self._buy_trader_weapon()
			return
	...
	def try_move_player(self, delta_x: int, delta_y: int) -> None:
		...
		for _ in range(steps_to_take):
			...
			if self.rest_floor and self.player_pos == self.trader_pos:
				self.trader_shop_open = True
				weapon_label = str(WEAPON_TYPES[self.trader_weapon]["label"]) if self.trader_weapon else "SOLD OUT"
				self.status_text = (
					f"Trader: [1] Health Potion ({TRADER_POTION_COST}g, full heal)  "
					f"[2] {weapon_label} ({TRADER_WEAPON_COST}g)  [Esc] Leave"
				)
	...
	def award_enemy_defeat(self, enemy: Enemy, archetype: dict[str, object]) -> bool:
		...
		passive_heal = 0
		if self.weapon_rarity >= 4:
			health_before = self.player_health
			self.player_health = min(self.max_health, self.player_health + HIGH_RARITY_KILL_HEAL)
			passive_heal = self.player_health - health_before
		if enemy.kind == BOSS_KIND:
			self.status_text = "The Abyss Titan falls. You escaped the dungeon."
			if passive_heal > 0:
				self.status_text += f" High-rarity passive restores {passive_heal} HP."
		...
		if passive_heal > 0:
			self.status_text = f"{self.status_text} High-rarity passive: +{passive_heal} HP."
	...
	def collect_item_if_present(self) -> None:
		...
		for item in list(self.items):
			...
			if item.kind == TREASURE:
				...
				self.player_health = min(self.max_health, self.player_health + POTION_HEAL_AMOUNT)
				self.status_text = f"You drink a potion and recover {POTION_HEAL_AMOUNT} health."
			return

		self.status_text = "You move deeper into the dungeon."

	def _buy_health_potion(self) -> None:
		"""Purchase a full-heal potion from the trader for TRADER_POTION_COST gold."""
		if self.score < TRADER_POTION_COST:
			self.status_text = f"Not enough gold! You need {TRADER_POTION_COST}g (have {self.score}g)."
			return
		self.score -= TRADER_POTION_COST
		self.player_health = self.max_health
		self.add_inventory_item(f"Trader Health Potion (full heal, {TRADER_POTION_COST}g)")
		self.sound_bank.play(self.sound_bank.pickup, 0.45)
		self.status_text = f"You buy a health potion for {TRADER_POTION_COST}g. HP fully restored to {self.max_health}!"

	def _buy_trader_weapon(self) -> None:
		"""Purchase the trader's high-level weapon for TRADER_WEAPON_COST gold."""
		if self.trader_weapon is None:
			self.status_text = "The trader has no weapons left to sell."
			return
		if self.score < TRADER_WEAPON_COST:
			self.status_text = f"Not enough gold! You need {TRADER_WEAPON_COST}g (have {self.score}g)."
			return
		self.score -= TRADER_WEAPON_COST
		weapon_label = str(WEAPON_TYPES[self.trader_weapon]["label"])
		rarity_name = RARITY_NAMES[self.trader_weapon_rarity]
		current_rarity = self.weapon_rarities.get(self.trader_weapon, 1)
		if self.trader_weapon_rarity > current_rarity:
			self.weapon_rarities[self.trader_weapon] = self.trader_weapon_rarity
			if self.equipped_weapon == self.trader_weapon:
				self.weapon_rarity = self.trader_weapon_rarity
		self.add_inventory_item(f"{weapon_label} ({rarity_name}) [Trader, {TRADER_WEAPON_COST}g]")
		self.sound_bank.play(self.sound_bank.level_up, 0.35)
		bought_weapon = self.trader_weapon
		self.trader_weapon = None
		self.status_text = (
			f"You buy {weapon_label} [{rarity_name}] for {TRADER_WEAPON_COST}g. "
			"The trader packs up the stall."
		)
		_ = bought_weapon  # consumed
	...
	def advance_floor(self) -> None:
		...
		if self.floor_number >= FINAL_FLOOR:
			...
			self.status_text = (
				f"Floor {self.floor_number}: A peaceful rest floor. "
				"A friendly trader awaits — walk to them to browse wares."
			)
		else:
	...
	def draw_trader(self) -> None:
		"""Draw the friendly trader NPC on rest floors."""
		if self.trader_pos[0] < 0:
			return
		center_x, center_y = self.tile_center(self.trader_pos[0], self.trader_pos[1])
		bob = math.sin(self.animation_time * 2.8) * 2.0
		# Ground shadow
		arcade.draw_ellipse_filled(center_x, center_y - 14, 22, 8, (0, 0, 0, 70))
		# Robe body
		arcade.draw_triangle_filled(
			center_x - 12, center_y - 12,
			center_x + 12, center_y - 12,
			center_x, center_y + 12 + bob,
			(68, 140, 68),
		)
		# Torso
		arcade.draw_lbwh_rectangle_filled(center_x - 7, center_y - 5 + bob, 14, 16, (92, 180, 92))
		# Head
		arcade.draw_circle_filled(center_x, center_y + 14 + bob, 9, (244, 220, 180))
		# Hat brim
		arcade.draw_lbwh_rectangle_filled(center_x - 11, center_y + 18 + bob, 22, 4, (160, 124, 48))
		# Hat top
		arcade.draw_triangle_filled(
			center_x - 8, center_y + 22 + bob,
			center_x + 8, center_y + 22 + bob,
			center_x, center_y + 32 + bob,
			(180, 140, 56),
		)
		# Eyes
		arcade.draw_circle_filled(center_x - 3, center_y + 14 + bob, 1.5, (40, 30, 20))
		arcade.draw_circle_filled(center_x + 3, center_y + 14 + bob, 1.5, (40, 30, 20))
		# Outline glow
		arcade.draw_lbwh_rectangle_outline(center_x - 14, center_y - 14, 28, 32, TRADER_COLOR, 2)
		# Floating label
		arcade.draw_text(
			"Trader",
			center_x,
	...
	def draw_hud(self) -> None:
		...
		if self.floor_number >= FINAL_FLOOR:
			...
			arcade.draw_text(
				"Rest Floor: Friendly trader nearby.",
				SCREEN_WIDTH - 20,
				HUD_HEIGHT - 38,
				TRADER_COLOR,
				heading_size - 2,
	...
	def draw_shop_overlay(self) -> None:
		"""Draw the trader shop purchase overlay."""
		box_w, box_h = 560, 270
		bx = SCREEN_WIDTH / 2 - box_w / 2
		by = SCREEN_HEIGHT / 2 - box_h / 2
		arcade.draw_lrbt_rectangle_filled(0, SCREEN_WIDTH, 0, SCREEN_HEIGHT, (0, 0, 0, 140))
		arcade.draw_lbwh_rectangle_filled(bx, by, box_w, box_h, (14, 26, 18, 252))
		arcade.draw_lbwh_rectangle_outline(bx, by, box_w, box_h, TRADER_COLOR, 2)
		arcade.draw_text(
			"Friendly Trader",
			SCREEN_WIDTH / 2, by + box_h - 38,
			TRADER_COLOR, 22, anchor_x="center", font_name="Arial", bold=True,
		)
		arcade.draw_text(
			f"Your Gold: {self.score}g",
			SCREEN_WIDTH / 2, by + box_h - 68,
			TREASURE_COLOR, 15, anchor_x="center", font_name="Arial",
		)
		# Potion row
		can_afford_potion = self.score >= TRADER_POTION_COST
		potion_label_color = (130, 240, 190) if can_afford_potion else (140, 140, 140)
		arcade.draw_text(
			f"[1]  Health Potion  \u2014  {TRADER_POTION_COST} gold",
			bx + 28, by + box_h - 108,
			potion_label_color, 15, font_name="Arial",
		)
		arcade.draw_text(
			f"Restores HP fully to {self.max_health}",
			bx + 48, by + box_h - 130,
			(160, 200, 178), 13, font_name="Arial",
		)
		# Weapon row
		if self.trader_weapon is not None:
			weapon_label = str(WEAPON_TYPES[self.trader_weapon]["label"])
			rarity_name = RARITY_NAMES[self.trader_weapon_rarity]
			rarity_color = RARITY_COLORS[self.trader_weapon_rarity]
			can_afford_weapon = self.score >= TRADER_WEAPON_COST
			weapon_text_color = rarity_color if can_afford_weapon else (140, 140, 140)
			arcade.draw_text(
				f"[2]  {weapon_label}  ({rarity_name})  \u2014  {TRADER_WEAPON_COST} gold",
				bx + 28, by + box_h - 168,
				weapon_text_color, 15, font_name="Arial",
			)
			blurb = str(WEAPON_TYPES[self.trader_weapon]["blurb"])
			arcade.draw_text(
				blurb,
				bx + 48, by + box_h - 190,
				(160, 200, 220), 13, font_name="Arial", width=int(box_w - 80),
			)
		else:
			arcade.draw_text(
				"[2]  Weapon \u2014 SOLD OUT",
				bx + 28, by + box_h - 168,
				(120, 120, 120), 15, font_name="Arial",
			)
		arcade.draw_text(
			"[Esc]  Leave shop",
			SCREEN_WIDTH / 2, by + 22,
			(180, 200, 185), 13, anchor_x="center", font_name="Arial",
		)
